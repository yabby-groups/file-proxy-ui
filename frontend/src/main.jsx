...
function App() {
  ...
  return <main className="shell">
    <header className="topbar"><div className="brand"><img src={logo} alt="" /><div><h1>Myna File Proxy <span className="versionBadge">v1.2.0.0</span></h1><p>{status.api_base_url}</p></div></div><div className="topActions"><div className="languageSwitch"><Languages size={16} /><button className={language === 'en' ? 'active' : ''} type="button" onClick={() => setLanguage('en')}>EN</button><button className={language === 'zh' ? 'active' : ''} type="button" onClick={() => setLanguage('zh')}>中文</button></div><button className="iconButton" type="button" onClick={() => setSettingsOpen((open) => !open)} disabled={!!busy} title={t('settings')}><Settings size={18} /></button></div></header>
    ...
    {!status.logged_in ? <section className="loginScreen"><form className="loginCard" onSubmit={(event) => { event.preventDefault(); void run(t('loginTask'), login); }}><div className="panelHeader"><LogIn size={20} /><h2>{t('login')}</h2></div><label>{t('username')}<input value={name} onChange={(event) => setName(event.target.value)} autoFocus={!totpRequired} /></label><label>{t('password')}<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} /></label>{totpRequired && <label>{t('authenticatorCode')}<input type="text" inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength="6" required value={totpCode} onChange={(event) => setTotpCode(event.target.value.replace(/\D/g, ''))} placeholder="123456" autoFocus /><small>{t('authenticatorCodeHint')}</small></label>}<button className="primary" type="submit" disabled={!!busy}><LogIn size={18} />{t('login')}</button></form></section> : <>
